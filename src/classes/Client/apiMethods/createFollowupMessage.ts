...
import { Client, CreateInteractionMessageData, FetchQueue, Message, RawMessageData, Role, RoleResolvable, User, UserResolvable } from "../../../internal";
import getRoute from "../../../util/getRoute";
import parseCreateMessageData, { ParsedCreateMessageData } from "../../../util/parseCreateMessageData";

export default async function createFollowupMessage(client: Client, interactionToken: string, createInteractionMessageData: CreateInteractionMessageData): Promise<Message> {

    // Resolve objects
    const allowedMentionsUsers: Array<string | undefined> | undefined = createInteractionMessageData.allowedMentions?.users?.map((u: UserResolvable) => User.resolveID(u));
    ...
    const allowedMentionsRoles: Array<string | undefined> | undefined = createInteractionMessageData.allowedMentions?.roles?.map((r: RoleResolvable) => Role.resolveID(r));
    ...
    // Parse payload data
    const data: ParsedCreateMessageData = await parseCreateMessageData(createInteractionMessageData, {
