...
export default async function addThreadMember(client: Client, channelResolvable: ChannelResolvable, userResolvable: UserResolvable): Promise<void> {
    ...
    if (client._cacheStrategies.permissions.enabled) {
        ...
        if ((threadCacheData.type === CHANNEL_TYPE_PRIVATE_THREAD) && (!client.hasPermission("USE_PRIVATE_THREADS", channelID))) throw new PermissionError({ permission: "USE_PRIVATE_THREADS" });
        else if ((threadCacheData.type !== CHANNEL_TYPE_PRIVATE_THREAD) && (!client.hasPermission("USE_PUBLIC_THREADS", channelID))) throw new PermissionError({ permission: "USE_PUBLIC_THREADS" });
    }
