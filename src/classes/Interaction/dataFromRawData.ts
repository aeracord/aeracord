import { AnyInteractionData, ComponentType, INTERACTION_TYPE_COMMAND, INTERACTION_TYPE_COMPONENT, Member, Message, RawInteractionData, RawMemberData, RawMessageData, User } from "../../internal";
...
export default function dataFromRawData(rawData: RawInteractionData): AnyInteractionData {
    ...
    // Parse button data
    else if (rawData.type === INTERACTION_TYPE_COMPONENT) interactionData = {
        id: rawData.id,
        type: rawData.type,
        token: rawData.token,
        applicationID: rawData.application_id,
        data: {
            type: rawData.data.component_type as ComponentType,
